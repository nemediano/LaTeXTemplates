...
Luego despejamos de la ecuación~\ref{eq:momento} a $\bar{w_n}$, y lo sustituimos en~\ref{eq:energia} y obtenemos el valor de $\bar{s_n}$. En realidad se obtienen dos valores $\bar{s_n} = \bar{u_n}$ y $\bar{s_n} = \bar{u_n} \frac{r-1}{r+1}$, tomamos el segundo (el primero corresponde a la condición inicial, por que las colisiones elásticas son reversibles en el tiempo), y lo sustituimos de nueva en~\ref{eq:momento} para obtener el valor de $\bar{w_n} = \frac{2 r \bar{u_n}}{r + 1}$.
