...
Los métodos nos deben de permitir editar estos cuatro miembros. En este sentido la clase \mintinline{cpp}{Particle} es muy simple pues solo contiene los métodos mutadores obvios.
...
\begin{minipage}{\linewidth}
  ...
  linenos,
  firstline=5, %If you omit this two fields, the whole file is pulled
  lastline=37
  ]{cpp}{../../../GLSamples/SoftBody/physics/Particle.h}
  \caption{Definición del objeto Partícula}
  \label{lst:bitManip2}
  \end{listing}
\end{minipage}
\par
...
Para representarlos se creó la clase \mintinline{cpp}{SpringDamper} que tiene como miembros \emph{referencias} a las dos particulas que une el resorte.
...
La clase \mintinline{cpp}{Face} tiene:
\begin{itemize}
...
Tambien se define una clase para representar el cuerpo flexible: \mintinline{cpp}{SoftBody}. Esta clase es la mas importante de la aplicación.
  
Finalmente, se necesita también una clase para representar el único cuerpo rígido en la escena. La clase \mintinline{cpp}{Ball} tiene como miembros una partícula, un escalar que representa el radio de la esfera y un miembro para registrar que método numérico se usa para integrar (De nuevo por IS, era conveniente que cada objeto en la escena llevara registro de que integrador está en uso).
