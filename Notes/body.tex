...
\begin{minipage}{\linewidth}
  ...
  fontsize=\footnotesize,
  linenos,
  firstline=48, %If you omit this two fields, the whole file is pulled
  lastline=59
...
The following code snippet shows a valid C++ sample of these operations:
...
Another important thing to notice is that, \emph{as long as the shift left significan digits} (or else we \emph{underflow}); shifting values to the right is equivalent to divide by $2^n$ where $n$ is the number of bits shifted.
...
These operator are the expected equivalent that the ones defined in first order logic.
Remember that they operate bitwise.
That is why they are different than the logical ones that are represented by double ampersand or double pipe.
