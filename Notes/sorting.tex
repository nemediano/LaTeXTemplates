...
\For{$i \gets b \textbf{ to } e-1 \textbf{ step } 1$} \label{merge:loop} \Comment{Merge the two halves in a temporal buffer $B$}
    ...
    \EndIf
\EndFor
\State $j \gets 0$ \Comment{A varible to keep track of the place in $B$}
...
Finally, since we have the ordered result in the temporat buffer $B$, wee need to copy it back to the original array $V$.
