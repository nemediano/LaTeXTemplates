...
\label{alg:siftdown}
\begin{algorithmic}[1] % El número le dice al entorno desde que número empezar a contar. Si le pones 0 omite los números
\Require $H$, $b$ and $e$ \Comment{A heap $H$, the indices $(h_b, h_e)$ that represents the subheap whose root changed}
\Ensure $H$ as a valid heap
\Procedure{SiftDown}{$H$, $b$, $e$} \Comment{$b$ stands for \emph{begin} and $e$ for \emph{end}}
\State $r \gets b$ \Comment{$r$ is the index of the \emph{current} root}
\While{$L_c(r) < e$} \Comment{while the root has at least one child}
    \State $s \gets r$ \Comment{$s$ is the index of the best candidate for swap ($r$ for now)}
    \State $l_c \gets L_c(r)$ \Comment{index of the left child}
    \If{$H(r) < H(l_c)$} \Comment{If you are smaller than left child}
        \State $s \gets l_c$ \Comment{left child is a better candidate to swap}
    \EndIf
    \State $r_c \gets R_c(r)$ \Comment{index of the right child}
    \If{$r_c < e$\textbf{ and }$H(r) < H(r_c)$} \Comment{If the right child exists and you are smaller than him}
        \State $s \gets r_c$ \Comment{right child is a better candidate to swap}
    \EndIf
    \If{$s = r$} \Comment{If the best candidate to swap is still $r$}
        \State \Return \Comment{then, there are no more possible swaps: we are done}
    \Else
        \State $H(r) \leftrightarrow H(s)$ \Comment{perform a swap}
        \State $r \gets s$ \Comment{move to the next subheap}
    \EndIf
\EndWhile
\EndProcedure
\end{algorithmic}
...
In the algorthm for sift down (Algorithm~\ref{alg:siftdown}) we try to swap the node at the root with one of his childs until we are greater than both of them.
...
\Procedure{heapify}{$V$} 
\State $c \gets P(v_{n-1})$ \Comment{$c$ is for current node. For now is the parent of the last element}
\While{$c \geq 0$} \Comment{while current node it not the root}
    \State \Call{SiftDown}{$V$, $c$, $n$} \Comment{fix the heap to the right of the current node}
    \State $c \gets c - 1$ \Comment{go to the next parent node}
\EndWhile
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
\begin{listing}
\inputminted[
  firstline=126, %If you omit this two fields, the whole file is pulled
  lastline=179
  ]{cpp}{src/sortAlgorithms.cpp}
  \caption{The \emph{sift down} and \emph{heapify} operations using arrays}\label{lst:siftAndHeapify}
\end{listing}

\begin{listing}
\inputminted[
  firstline=109, %If you omit this two fields, the whole file is pulled
  lastline=189
  ]{cpp}{src/sortAlgorithms.h}
  \caption{The \emph{sift down} and \emph{heapify} operations using iterators}\label{lst:siftAndHeapifyIter}
\end{listing}
...
However, with iterators we take the usual convention of returning the \mintinline{cpp}{end} iterator to indicate that such an element does not exist.
...
First, we find the maximum element of the array, then we swap it with the last element of the array and we \say{shrink} the array by one element.
...
\begin{enumerate}
 ...
 \item The swap from the last step  has just ruined the heap from the root. Therefore, we fix it by calling \mintinline{cpp}{sift_down}
 \item Since the maximum element it's now at the end, we can \say{shrink} the array by moving the \say{end mark} to the left.
...
\Procedure{HeapSort}{$V$}
\State \Call{Heapify}{$V$}  \Comment{start by creating a valid in place heap of the array}
\State $l \gets n - 1$ \Comment{$l$ is the index the \emph{last} element}
\While{$l > 0$} \Comment{while the last node it's not the root}
    \State $v_0 \leftrightarrow v_l$ \Comment{the first element is the largest put it in his place}
    \State \Call{SiftDown}{$V$, $0$, $l$} \Comment{the swap ruined the heap, restore it}
    \State $l \gets l - 1$ \Comment{the heap size is reduced by one}
\EndWhile
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
\begin{listing}
\inputminted[
  firstline=181, %If you omit this two fields, the whole file is pulled
  lastline=194
  ]{cpp}{src/sortAlgorithms.cpp}
  \caption{An implementation of \emph{heapsort} using arrays}\label{lst:heapsortArray}
\end{listing}

\begin{listing}
\inputminted[
  firstline=191, %If you omit this two fields, the whole file is pulled
  lastline=207
  ]{cpp}{src/sortAlgorithms.h}
  \caption{An implementation of \emph{heapsort} using iterators}\label{lst:heapsortIter}
\end{listing}
