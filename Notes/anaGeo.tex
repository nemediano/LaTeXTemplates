...
The Equation~\ref{eq:proj} puts in evidence that $\mathbf{y}$ is a directional vector and that $\frac{(\mathbf{x} \cdot \mathbf{y})}{|\mathbf{y}|^2}$ it his scaling lenght in units of $\mathbf{y}$.
...
While the previos sections results are valid to any dimenion $\mathbb{R}^n$, the result in this Section~\ref{sec:ray2segment} is constrained to $\mathbb{R}^2$.
...
Which is a linear equation system that we can solve to get the values of $s$ and $t$. For example, by Cramer's rule:
\begin{align*}
