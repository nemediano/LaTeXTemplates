...
\begin{minipage}{\linewidth}
  \begin{algorithm}[H]
    ...
    \begin{algorithmic}[1] % The number tells where the line numbering should start 0 for no number
      ...
      \If{$|\mathbf{v}| = 0$} \Comment{There is no ray}
      ...
      \If{$\Delta \neq 0$} \Comment{Ray and segment are not parralell. Perform normal test}
          ...
          \Else \Comment{They do not intersect}
            \State \textbf{return} false
          \EndIf
      \Else \Comment{Ray and segment are parallel. Handle special cases}
      \EndIf
      \If{$\Delta_s \neq 0$} \Comment{They are parallel but they are not in the same line}
        \State \textbf{return} false
      \Else \Comment{They are on the same line. Check relative positions of $\mathbf{o}$ and $\overline{\mathbf{a}\mathbf{b}}$}
        \State $s_p \gets \frac{(\mathbf{o} - \mathbf{a}) \cdot (\mathbf{b} - \mathbf{a})}{|\mathbf{b} - \mathbf{a}|^2}$
        \State \textbf{return} $s_p \leq 1$
      \EndIf
      \EndProcedure
    \end{algorithmic}
  \end{algorithm}
