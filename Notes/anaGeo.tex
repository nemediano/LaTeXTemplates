...
\begin{itemize}
  \item $\mathbf{R}_1$ hits the whole segment.
  \item $\mathbf{R}_2$ misses the segment.
  \item $\mathbf{R}_3$ hits the segment in the middle.
  \item $\mathbf{R}_4$ kisses (touches) the segmnet  at his endpoint. By convention we will say this is a hit.
...
Another interesting observation, is that if we identify the last equallity with the equations of the ray and the segement; 
we see that the matrix part (right hand side) is  concerned with the linear independence of the directional vectors.
Meanwhile, the vector part (left hand side) is concerned with the linear independence of the origin points.
...
When $\Delta = 0$ means that we have linear depndency of the vectors $\mathbf{u}$ and $\mathbf{v} = \mathbf{b} - \mathbf{a}$.
If we look again at Figure~\ref{fig:rays2seg} this means that the ray and the segmen are parallel.
There is again two possible cases geometric situation for this. One of them also divides in sub cases: 
\begin{enumerate}
  \item They are parrallel but they are not on the same line. Like the case of $\mathbf{R}_5$ in the Figure~\ref{fig:rays2seg}.
  \item They are parrallel and the reside on the same line. Like $\mathbf{R}_1$ in the Figure~\ref{fig:rays2seg}.
  \begin{enumerate}
    \item They are parallel, on the same line but the do not intersect. Since $\mathbf{R}$ and $\mathbf{S}$ are not complete lines.
    \item They are parallel, on the same line and they have some infinite ovelaping. This is the sub case of $\mathbf{R}_1$ in the Figure~\ref{fig:rays2seg}.
  \end{enumerate}
\end{enumerate}
